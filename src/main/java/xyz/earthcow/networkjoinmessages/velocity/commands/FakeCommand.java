...
public class FakeCommand implements SimpleCommand {
    ...
    public void execute(SimpleCommand.Invocation invocation) {
        ...
        if (args.length < 1) {
            String msg = VelocityMain.getInstance()
                .getRootNode()
                .node("Messages", "Commands", "Fakemessage", "NoArgument")
                .getString(
                    "&6Arguments:\n" +
                    "- &cfakejoin\n" +
                    "- &cfakequit\n" +
            ...
            if (
                ...
                if (!player.hasPermission("networkjoinmessages.silent")) {
                    ...
                    player.sendMessage(
                        Component.text(HexChat.translateHexCodes(msg))
                    );
                    return;
                } else {
    ...
    public boolean hasPermission(final SimpleCommand.Invocation invocation) {
        return invocation
            .source()
            .hasPermission("networkjoinmessages.fakemessage");
    }
            ...
            default:
                return ImmutableList.of(
                    VelocityMain.getInstance()
                        .getRootNode()
                        .node("Messages", "Commands", "NoMoreArgumentsNeeded")
                        .getString("No more arguments needed.")
                );
        }
    }
}
