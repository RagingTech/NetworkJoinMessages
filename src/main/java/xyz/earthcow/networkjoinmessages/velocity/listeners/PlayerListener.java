...
public class PlayerListener {
    ...
    private final String silent = VelocityMain.getInstance()
        .getRootNode()
        .node("Messages", "Misc", "SilentPrefix")
        .getString("&7[Silent] ");
        ...
        if (Storage.getInstance().isSwapServerMessageEnabled()) {
            ...
            if (Storage.getInstance().getAdminMessageState(player)) {
                VelocityMain.getInstance()
                    .SilentEvent("MOVE", player.getUsername(), from, to);
                if (Storage.getInstance().notifyAdminsOnSilentMove()) {
                    Component silentMessage = Component.text(
                        HexChat.translateHexCodes(silent + message)
                    );
                    for (Player p : VelocityMain.getInstance()
                        .getProxy()
                        .getAllPlayers()) {
    ...
    public void onLogin(LoginEvent event) {
        ...
        ScheduledTask task = VelocityMain.getInstance()
            .getProxy()
            .getScheduler()
                ...
                if (player.isActive()) {
                        ...
                        if (
                            VelocityMain.getInstance()
                                ...
                                .node(
                                    "OtherPlugins",
                                    "PremiumVanish",
                                    "ToggleFakemessageWhenVanishing"
                                )
                                .getBoolean(false)
                        ) {
                    ...
                    if (Storage.getInstance().getAdminMessageState(player)) {
                        if (
                            ...
                            String toggleNotif = VelocityMain.getInstance()
                                .getRootNode()
                                .node(
                                    "Messages",
                                    "Commands",
                            ...
                            player.sendMessage(
                                Component.text(
                                    HexChat.translateHexCodes(toggleNotif)
                                )
                            );
                        }
                        ...
                        // Send to console
                        VelocityMain.getInstance()
                            .SilentEvent("JOIN", player.getUsername());
                    ...
                    VelocityMain.getInstance()
                        .getProxy()
                        .getEventManager()
                        .fireAndForget(networkJoinEvent);
                }
            })
            .delay(
                VelocityMain.getInstance()
                    .getRootNode()
                    .node("Messages", "Misc", "JoinMessageDelaySeconds")
                    .getInt(3),
                TimeUnit.SECONDS
            )
            .schedule();
    ...
    public void onDisconnect(DisconnectEvent event) {
        ...
        if (Storage.getInstance().getAdminMessageState(player)) {
            VelocityMain.getInstance()
                .SilentEvent("QUIT", player.getUsername());
            if (Storage.getInstance().notifyAdminsOnSilentMove()) {
                Component silentMessage = Component.text(
                    HexChat.translateHexCodes(silent + message)
                );
        ...
        VelocityMain.getInstance()
            .getProxy()
            .getEventManager()
            .fireAndForget(networkQuitEvent);
    }
}
