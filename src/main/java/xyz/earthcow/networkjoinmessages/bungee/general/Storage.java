...
public class Storage {
    ...
    public void setUpDefaultValuesFromConfig() {
        this.SwapServerMessageEnabled = BungeeMain.getInstance()
            .getConfig()
            .getBoolean("Settings.SwapServerMessageEnabled", true);
        this.JoinNetworkMessageEnabled = BungeeMain.getInstance()
            .getConfig()
            .getBoolean("Settings.JoinNetworkMessageEnabled", true);
        this.LeaveNetworkMessageEnabled = BungeeMain.getInstance()
            .getConfig()
            .getBoolean("Settings.LeaveNetworkMessageEnabled", true);
        this.NotifyAdminsOnSilentMove = BungeeMain.getInstance()
            .getConfig()
            .getBoolean("Settings.NotifyAdminsOnSilentMove", true);

        this.SwapViewableByJoined = BungeeMain.getInstance()
            .getConfig()
            .getBoolean(
                "Settings.SwapServerMessageViewableBy.ServerJoined",
                true
            );
        this.SwapViewableByLeft = BungeeMain.getInstance()
            .getConfig()
            .getBoolean(
                "Settings.SwapServerMessageViewableBy.ServerLeft",
                true
            );
        this.SwapViewableByOther = BungeeMain.getInstance()
            .getConfig()
            .getBoolean(
                "Settings.SwapServerMessageViewableBy.OtherServer",
                true
            );

        this.JoinViewableByJoined = BungeeMain.getInstance()
            .getConfig()
            .getBoolean(
                "Settings.JoinNetworkMessageViewableBy.ServerJoined",
                true
            );
        this.JoinViewableByOther = BungeeMain.getInstance()
            .getConfig()
            .getBoolean(
                "Settings.JoinNetworkMessageViewableBy.OtherServer",
                true
            );

        this.LeftViewableByLeft = BungeeMain.getInstance()
            .getConfig()
            .getBoolean(
                "Settings.LeaveNetworkMessageViewableBy.ServerLeft",
                true
            );
        this.LeftViewableByOther = BungeeMain.getInstance()
            .getConfig()
            .getBoolean(
                "Settings.LeaveNetworkMessageViewableBy.OtherServer",
                true
            );

        //Blacklist
        this.BlacklistedServers = BungeeMain.getInstance()
            .getConfig()
        ...
        this.useBlacklistAsWhitelist = BungeeMain.getInstance()
            .getConfig()
            .getBoolean("Settings.UseBlacklistAsWhitelist", false);
        this.SwapServerMessageRequires = BungeeMain.getInstance()
            .getConfig()
            .getString("Settings.SwapServerMessageRequires", "ANY")
            .toUpperCase();

        this.ServerJoinMessageDisabled = BungeeMain.getInstance()
            .getConfig()
            .getStringList("Settings.IgnoreJoinMessagesList");
        this.ServerLeaveMessageDisabled = BungeeMain.getInstance()
            .getConfig()
    ...
    public boolean getAdminMessageState(ProxiedPlayer p) {
        if (p.hasPermission("networkjoinmessages.silent")) {
            if (messageState.containsKey(p.getUniqueId())) {
                ...
                boolean state = BungeeMain.getInstance()
                    .getConfig()
                    .getBoolean("Settings.SilentJoinDefaultState", true);
    ...
    public List<ProxiedPlayer> getSwitchMessageReceivers(
        String to,
        String from
    ) {
    ...
    public List<ProxiedPlayer> getServerPlayers(String serverName) {
        ServerInfo info = BungeeMain.getInstance()
            .getProxy()
            .getServers()
            .get(serverName);
        if (info != null) {
