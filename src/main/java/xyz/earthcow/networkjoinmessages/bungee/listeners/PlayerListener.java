...
public class PlayerListener implements Listener{
	...
	String silent = BungeeMain.getInstance().getConfig().getString("Messages.Misc.SilentPrefix",
			"&7[Silent] ");
	...
	public void onPlayerSwitchServer(ServerConnectedEvent e) {
		...
		if(Storage.getInstance().isSwapServerMessageEnabled()) {
	    	...
	    	if(Storage.getInstance().getAdminMessageState(player)) {
	    		BungeeMain.getInstance().SilentEvent("MOVE", player.getName(), from, to);
			...
			BungeeMain.getInstance().getProxy().getPluginManager().callEvent(swapServerEvent);
		}
    ...
    public void onPostLogin(PostLoginEvent event) {
   	 ...
   	 ProxyServer.getInstance().getScheduler().schedule(BungeeMain.getInstance().getPlugin(), new Runnable() {
		 public void run()
		 {
			 if(player.isConnected()) {
				 while (player.getServer() == null) {
					 try {
						 BungeeMain.getInstance().getLogger().warning(player.getName() + "'s SERVER IS NULL WAITING A SECOND!!");
						 wait(1000);
		    		...
		    		if(BungeeMain.getInstance().VanishAPI) {
		    			if(BungeeMain.getInstance().getConfig().getBoolean("OtherPlugins.PremiumVanish.ToggleFakemessageWhenVanishing",false))
			    	...
			    	if(Storage.getInstance().getAdminMessageState(player)) {
			    		...
			    		//Send to console
			    		BungeeMain.getInstance().SilentEvent("JOIN", player.getName());
					...
					// All checks have passed to reach this point
				 	...
				 	BungeeMain.getInstance().getProxy().getPluginManager().callEvent(networkJoinEvent);
			 }


		 }
	 }, BungeeMain.getInstance().getConfig().getInt("Messages.Misc.JoinMessageDelaySeconds", 3), TimeUnit.SECONDS);
...
//        }
    ...
    public void onPostQuit(PlayerDisconnectEvent event) {
		...
		BungeeMain.getInstance().getProxy().getPluginManager().callEvent(networkQuitEvent);
    }
}
